import { Download, ImageUp, Link2, RefreshCw, X } from "lucide-react";
...
export function UploadWidgetUploadItem({ uploadId, upload }: UploadWidgetUploadItemProps) {
    ...
    return (
        <motion.div
            className="p-3 rounded-lg flex flex-col gap-3 shadow-shape-content bg-white-opacity-2 relative overflow-hidden"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        ...
        >
            <div className="flex flex-col gap-1">
                ...
                <span className="text-tiny text-zinc-400 flex gap-1.5 items-center">
            ...
            >
                <Progress.Indicator
                    className="bg-indigo-500 h-1 grou-data-[status=success]:bg-green-400 group-data-[status=error]:bg-red-400 group-data-[status=canceled]:bg-yellow-400 transition-all"
            ...
            <div className="absolute top-2 right-2 flex items-center gap-1">
                ...
                <Button disabled={!['canceled', 'error'].includes(upload.status)} onClick={() => retryUpload(uploadId)} size="icon-sm">
